...
class exports.ItinCard extends Layer
...
############################ CONSTRUCTOR    ############################
	constructor: (@options = {}) ->
		...
		# moment.relativeTimeRounding(Math.ceiling)
		...
		switch type
			...
			when "offboarding"
				...
				return "<div class='itineraryInfoBlock'>
								...
								<div class = 'timeBlock'>
									<div class='itineraryArrow'></div>
									<div class='app itineraryTime'>Alle von Bord um #{moment(segment.allFromBoard).format('LT')}</div>
								</div>
							</div>"
...
##### GETTERS / SETTERS ######
	...
	@define 'segment',
		get: ->
			if @options.segment==""
				console.error("must define a segment")
			else
				@options.segment
		set: (value) ->
